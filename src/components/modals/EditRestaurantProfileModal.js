...
const EditProfileModal = (props) => {
  ...
  const uploadRestaurantAssets = () => {
    let promises = [];

    if (profileImageFile.current.files.length > 0) {
      const updateRestaurantProfileImagePromise = new Promise(
        (resolve, reject) => {
          ...
          const profileImageFileRef = ref(
            profileStorageRef,
            `${profileFileName}.${extension}`
          );
      ...
      promises.push(updateRestaurantProfileImagePromise);
    }

    if (backgroundImageFile.current.files.length > 0) {
      const updateRestaurantBgImagePromise = new Promise((resolve, reject) => {
        const storageRef = ref(storage);
        ...
        const restaurantStorageRef = ref(restaurantsStorageRef, slug);
        const profileStorageRef = ref(restaurantStorageRef, "Profile");
        const bgImageFileRef = ref(
          profileStorageRef,
          `${bgFileName}.${extension}`
        );
      ...
      promises.push(updateRestaurantBgImagePromise);
    }
    ...
    if (phoneNumber && validatePhoneNumber(phoneNumber)) {
      ...
      setDoc(
        ...
        .then(() => {
          setLoading(false);
          toast.success(
            "Update successful, please refresh this page to see changes"
          );
        })
        .catch(() => {
          setLoading(false);
          ...
          <div className="form-row">
            ...
            <Form.Group className="col-md-12">
              ...
              <Form.File
                ...
                accept="image/*"
                onChange={() =>
                  setProfileFileName(profileImageFile.current.files[0]?.name)
                }
              />
            </Form.Group>
            <Form.Group className="col-md-12">
              ...
              <Form.File
                ...
                onChange={() =>
                  setBackgroundFileName(
                    backgroundImageFile.current.files[0].name
                  )
                }
              />
            </Form.Group>
