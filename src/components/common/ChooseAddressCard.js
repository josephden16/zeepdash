        ...
        <Media>
          ...
          <div className="media-body">
            ...
            <p className="mb-0 text-black font-weight-bold">
              <button
                onClick={selectDeliveryLocation}
                className="btn btn-sm btn-success mr-2"
              >
                {props.addressData === props.selectedDeliveryLocation
                  ? "DELIVERING HERE"
                  : "DELIVER HERE"}
              </button>
            </p>
...
const Icon = ({ className, icon }) => {
  if (icon === "home") {
    return <RiHome4Fill className={className} />;
  }

  if (icon === "briefcase") {
    return <BsBriefcaseFill className={className} />;
  }
